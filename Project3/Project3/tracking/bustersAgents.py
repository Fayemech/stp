...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for dist in livingGhostPositionDistributions :
            localpos = None;
            localprob = 0;
            for curpos, curprob in dist.items() :
                if curprob > localprob :
                    localpos = curpos;
                    localprob = curprob;
                    
            for action in legal :
                successorP = Actions.getSuccessor(pacmanPosition, action);
                dis = self.distancer.getDistance(localpos, successorP);
                if resdis > dis :
                    res = action;
                    resdis = dis;
