    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None :
            ...
            for p in self.legalPositions :
                trueP = util.manhattanDistance(p, pacmanPosition);
                if emissionModel[trueP] > 0 :
                    allPossible[p] = self.beliefs[p] * emissionModel[trueP];
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None :
            ...
            for particles in self.particles :
                dis = util.manhattanDistance(particles, pacmanPosition);
                if emissionModel[dis] > 0 :
                    allPossible[particles] += emissionModel[dis];
                    sign = True;
            if not sign :
                self.initializeUniformly(gameState);
            else :
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        fullpermutation = list(itertools.product(poslist, repeat = cout));
        for i in range(self.numParticles) :
            self.particles.append(fullpermutation[i % len(fullpermutation)]);
